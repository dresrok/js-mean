...
type ObjectIdSchema = z.infer<typeof ObjectIdSchema>;
const ObjectIdSchema = z.string().refine(
  (val) => {
    // Validate if the string is a valid ObjectId format
  ...
  // Transform the validated string into an ObjectId instance
  return new Types.ObjectId(val);
});

type IncidentCreateSchema = z.infer<typeof incidentCreateSchema>;
const incidentCreateSchema = z
  .object({
    title: z.string().trim().min(8).max(120),
    description: z.string().trim().min(1).max(4000),
    severity: z.enum(['low', 'medium', 'high', 'critical']),
    status: z.enum(['new', 'ack', 'in_progress', 'resolved', 'closed', 'cancelled']),
    reportedBy: ObjectIdSchema,
    assignedTo: z.union([ObjectIdSchema, z.null()]).transform((val) => val === null ? undefined : val),
...
export async function listIncidents(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const incidents = await incidentService.getAllIncidentsOptimized(filters);
...
export async function createIncident(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const parsedPayload: ZodSafeParseResult<IncidentCreateSchema> = incidentCreateSchema.safeParse(req.body);
    ...
    // Invalidar caché después de crear un incidente
    invalidateCache();
...
export async function updateIncident(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (hasPermission(user.role, PERMISSIONS.INCIDENTS_UPDATE_ALL)) {
      ...
      // Invalidar caché después de actualizar un incidente
      invalidateCache();
...
export async function cancelIncident(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    // Invalidar caché después de cancelar un incidente
    invalidateCache();
...
export async function deleteIncident(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    // Invalidar caché después de eliminar un incidente
    invalidateCache();
